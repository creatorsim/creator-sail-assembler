...
function run(args) {
  ...
  function doRun() {
    ...
    FS.writeFile('my_code_v2.o' /*'output.o'*/, objfile);
    ...
    callMain(args);
    const fsfiles = FS.readdir('./');
    console.log("Directorio:", fsfiles);
...
function preprocess_ld(objectfile, linkerfile){
  outpufile = undefined;
  console.log("Estoy en el preprocesado del ld");
  objfile = objectfile;
  linkfile = linkerfile;

  run(["-T", "linker.ld", "-o", "output.elf", "my_code_v2.o"]);
  return outpufile;
...
console.log("He cargado el ld-new");
