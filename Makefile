...
BINUTILS_TARGET_FLAGS := --with-expat=yes --with-libexpat-prefix=/home/juancarlos/.emscripten_cache/sysroot $(BINUTILS_TARGET_FLAGS_EXTRA)
BINUTILS_NATIVE_FLAGS := $(BINUTILS_NATIVE_FLAGS_EXTRA)
GDB_TARGET_FLAGS := --with-expat=yes --with-libexpat-prefix=/home/juancarlos/.emscripten_cache/sysroot $(GDB_TARGET_FLAGS_EXTRA)
...
stamps/install-host-gcc: $(GCC_SRCDIR) 
...
stamps/build-binutils-linux: $(BINUTILS_SRCDIR) $(PREPARATION_STAMP)
	rm -rf $@ $(notdir $@)
	mkdir $(notdir $@)
# CC_FOR_TARGET is required for the ld testsuite.
	cd $(notdir $@) && CC_FOR_TARGET=$(GLIBC_CC_FOR_TARGET) $</configure \
		...
		--with-libexpat-prefix=/home/juancarlos/.emscripten_cache/sysroot \
		--disable-gdb \
		--disable-sim \
...
stamps/build-gdb-linux: $(GDB_SRCDIR) $(PREPARATION_STAMP)
	rm -rf $@ $(notdir $@)
	mkdir $(notdir $@)
# CC_FOR_TARGET is required for the ld testsuite.
	cd $(notdir $@) && CC_FOR_TARGET=$(GLIBC_CC_FOR_TARGET) $</configure \
...
stamps/build-binutils-linux-native: $(BINUTILS_SRCDIR) stamps/build-gcc-linux-stage2 $(PREPARATION_STAMP)
	rm -rf $@ $(notdir $@)
	mkdir $(notdir $@)
	cd $(notdir $@) && $</configure \
		--host=wasm64-unknown-emscripten \
...
stamps/build-binutils-newlib: $(BINUTILS_SRCDIR) $(PREPARATION_STAMP)
	rm -rf $@ $(notdir $@)
	mkdir $(notdir $@)
# CC_FOR_TARGET is required for the ld testsuite.
...
stamps/build-gdb-newlib: $(GDB_SRCDIR) $(PREPARATION_STAMP)
	rm -rf $@ $(notdir $@)
	mkdir $(notdir $@)
# CC_FOR_TARGET is required for the ld testsuite.
	cd $(notdir $@) && CC_FOR_TARGET=$(NEWLIB_CC_FOR_TARGET) $</configure \
...
stamps/build-binutils-musl: $(BINUTILS_SRCDIR) $(PREPARATION_STAMP)
	rm -rf $@ $(notdir $@)
	mkdir $(notdir $@)
# CC_FOR_TARGET is required for the ld testsuite.
...
stamps/build-spike: $(SPIKE_SRCDIR) $(PREPARATION_STAMP)
	rm -rf $@ $(notdir $@)
	mkdir $(notdir $@)
	cd $(notdir $@) && $</configure \
		--host=wasm64-unknown-emscripten \
...
stamps/build-qemu: $(QEMU_SRCDIR) $(PREPARATION_STAMP)
	rm -rf $@ $(notdir $@)
	mkdir $(notdir $@)
	cd $(notdir $@) && $</configure \
		--host=wasm64-unknown-emscripten \
...
stamps/build-llvm-linux: $(LLVM_SRCDIR) $(BINUTILS_SRCDIR) \
...
stamps/build-llvm-newlib: $(LLVM_SRCDIR) $(BINUTILS_SRCDIR)  \
...
stamps/build-dejagnu: $(DEJAGNU_SRCDIR) $(PREPARATION_STAMP)
	rm -rf $@ $(notdir $@)
	mkdir $(notdir $@)
	cd $(notdir $@) && $</configure \
		--prefix=$(INSTALL_DIR)
